...
def show_stats(update, context):
    ...
    update.message.reply_text(
        ...
        f"💎 Итог: {income - expense:,.0f} ₽".replace(',', ' '),
        parse_mode='Markdown', reply_markup=main_keyboard
    )
...
def show_employees(update, context):
    ...
    text = "👥 *СОТРУДНИКИ*\n\n"
    for e in EMPLOYEES:
        text += f"• {e}: +{stats[e]['inc']:,.0f} / -{stats[e]['exp']:,.0f} = {stats[e]['inc']-stats[e]['exp']:,.0f} ₽\n".replace(',', ' ')
    update.message.reply_text(text, parse_mode='Markdown', reply_markup=main_keyboard)
...
def edit_start(update, context):
    ...
    for o in ops[:10]:
        ...
        kb.append([InlineKeyboardButton(btn[:40], callback_data=f"edit_{o['id']}")])
    kb.append([InlineKeyboardButton("🔙 Отмена", callback_data="edit_cancel")])
    update.message.reply_text("✏️ *Выберите операцию:*", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
...
def edit_callback(update, context):
    ...
    if data.startswith("edit_"):
        ...
        if op['type'] == 'income':
            text = f"📌 *Продажа #{op_id}*\n{op['parfum_name']} {op['volume']} x{op['quantity']}\n👤 {op['employee']}\n💰 {op['amount']:,.0f} ₽".replace(',', ' ')
        else:
            text = f"📌 *Расход #{op_id}*\n{op['description']}\n👤 {op['employee']}\n💰 {op['amount']:,.0f} ₽".replace(',', ' ')
        ...
        query.edit_message_text(f"⚠️ Удалить #{op_id}?", reply_markup=kb)
    elif data.startswith("yes_"):
        op_id = int(data.split('_')[1])
        delete_operation(op_id)
        query.edit_message_text(f"✅ #{op_id} удален")
    elif data == "back":
...
def handle_message(update, context):
    ...
    if 'edit_id' in context.user_data:
        try:
            ...
            ops = get_all_operations()
            for o in ops:
                if o['id'] == context.user_data['edit_id']:
                    o['amount'] = new_sum
                    update_operation(o['id'], o)
                    break
    ...
    if chat_id in user_data:
        ...
        if state['type'] == 'income':
            step = state['state']
            if step == INCOME_STATES['NAME']:
                income_name(update, context)
            elif step == INCOME_STATES['VOLUME']:
                income_volume(update, context)
            elif step == INCOME_STATES['QUANTITY']:
                income_quantity(update, context)
            elif step == INCOME_STATES['EMPLOYEE']:
                income_employee(update, context)
            elif step == INCOME_STATES['PAYMENT']:
                income_payment(update, context)
            elif step == INCOME_STATES['BANK']:
                income_bank(update, context)
            elif step == INCOME_STATES['AMOUNT']:
                income_amount(update, context)
        elif state['type'] == 'expense':
            step = state['state']
            if step == EXPENSE_STATES['AMOUNT']:
                expense_amount(update, context)
            elif step == EXPENSE_STATES['DESCRIPTION']:
                expense_description(update, context)
            elif step == EXPENSE_STATES['EMPLOYEE']:
